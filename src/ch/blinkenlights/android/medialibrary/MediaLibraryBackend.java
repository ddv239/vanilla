...
public class MediaLibraryBackend extends SQLiteOpenHelper {
	...
	/**
	 * Regexp to detect costly oneartist_id queries which we can optimize
	 */
	private static final Pattern sQueryMatchIndividualArtistSearch = Pattern.compile("(^|.+ )" + MediaLibrary.ContributorColumns.ONEARTIST_ID + "=(\\d+)$");
	/**
	 * Regexp to detect costly albumartist_id queries which we can optimize
	 */
	private static final Pattern sQueryMatchAlbArtistSearch = Pattern.compile("(^|.+ )" + MediaLibrary.ContributorColumns.ALBUMARTIST_ID + "=(\\d+)$");
	/**
	 * Regexp to detect costly composer_id queries which we can optimize
	 */
	 ...
	 *
	 * @param dbh the writable database handle
	 */
	@Override
	...
	/**
	 * Marks all songs as 'deletable' - but doesn't delete them yet.
	 ...
	 */
	Cursor query(boolean distinct, String source, String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy, String limit) {

		if (MediaLibrary.VIEW_SONGS_ALBUMS_ARTISTS_HUGE.equals(source)) {
			Log.v("VanillaMusic", "+++ warning : using HUGE view in query!");
		}

		if (selection != null) {
			if (MediaLibrary.VIEW_SONGS_ALBUMS_ARTISTS.equals(source)) {
			...
			if (MediaLibrary.VIEW_ALBUMS_ARTISTS.equals(source)) {
			...
			if (genreMatch.matches()) {
				...
				if (source.equals(MediaLibrary.VIEW_SONGS_ALBUMS_ARTISTS) ||
						source.equals(MediaLibrary.VIEW_SONGS_ALBUMS_ARTISTS_HUGE)) {
					selection += MediaLibrary.SongColumns._ID + " IN (" + songsQuery + ") ";
				}

				if (source.equals(MediaLibrary.VIEW_ALBUMS_ARTISTS)) {
					selection += MediaLibrary.AlbumColumns._ID + " IN (" +
							buildTargetIdFromGenreSelect(MediaLibrary.SongColumns.ALBUM_ID, MediaLibrary.VIEW_SONGS_ALBUMS_ARTISTS, songsQuery) + ") ";
				}

				if (source.equals(MediaLibrary.VIEW_ARTISTS)) {
					selection += MediaLibrary.ContributorColumns.ARTIST_ID + " IN (" +
							buildTargetIdFromGenreSelect(MediaLibrary.ContributorColumns.ARTIST_ID, MediaLibrary.VIEW_SONGS_ALBUMS_ARTISTS, songsQuery) + ") ";
				}

				if (source.equals(MediaLibrary.VIEW_ALBUMARTISTS)) {
					selection += MediaLibrary.ContributorColumns.ALBUMARTIST_ID + " IN (" +
							buildTargetIdFromGenreSelect(MediaLibrary.ContributorColumns.ALBUMARTIST_ID, MediaLibrary.VIEW_SONGS_ALBUMS_ARTISTS_HUGE, songsQuery) + ") ";
					Log.v("VanillaMusic", "+++ warning: huge genrequery for albumartist!");
				}

				if (source.equals(MediaLibrary.VIEW_COMPOSERS)) {
					selection += MediaLibrary.ContributorColumns.COMPOSER_ID + " IN (" +
							buildTargetIdFromGenreSelect(MediaLibrary.ContributorColumns.COMPOSER_ID, MediaLibrary.VIEW_SONGS_ALBUMS_ARTISTS_HUGE, songsQuery) + ") ";
		...
		if (DEBUG)
			debugQuery(distinct, source, columns, selection, selectionArgs, groupBy, having, orderBy, limit);

		Cursor cursor = getReadableDatabase().query(distinct, source, columns, selection, selectionArgs, groupBy, having, orderBy, limit);
		if (cursor != null) {
	...
	private String[] extractVirtualColumn(String sql) {
		final Pattern[] pattern = new Pattern[]{sQueryMatchIndividualArtistSearch, sQueryMatchComposerSearch, sQueryMatchAlbArtistSearch};
		final int[] roles = {MediaLibrary.ROLE_INDIVIDUALARTIST, MediaLibrary.ROLE_COMPOSER, MediaLibrary.ROLE_ALBUMARTIST};
	 ...
	 * @param source      the table or view to query
	 * @param genreSelect the select string generated by buildSongIdFromGenreSelect
	 * @return an SQL string
	 */
	private String buildTargetIdFromGenreSelect(String target, String source, String genreSelect) {
		return "SELECT " + target + " FROM " + source + " WHERE "
